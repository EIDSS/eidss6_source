...
namespace bv.common.Configuration
{
    ...
    public class ConnectionCredentials : IComparable
    {
             ...
             string enryptedPassword, int? intMaxPoolSize = null)
        {
            ...
            m_SQLDatabase = database;
            m_SQLServer = server;
            m_SQLUser = Cryptor.Decrypt(encryptedUser, BaseSettings.CryptAlgorithmMode);
            if ((!string.IsNullOrEmpty(enryptedPassword)) && (!string.IsNullOrEmpty(m_SQLUser)))
            {
                m_SQLPassword = Cryptor.Decrypt(enryptedPassword, m_SQLUser, BaseSettings.CryptAlgorithmMode);
            }
        ...
        private void SaveToRegistry()
        {
            ...
            if (sqlUserRegistry.Length > 0)
            {
                ...
                if (s.Length > 0)
                {
                    sqlUserDecrypt = Cryptor.Decrypt(s, BaseSettings.CryptAlgorithmMode);
                }

                bool userChanged = false;
                if (m_SQLUser != null && !m_SQLUser.Equals(sqlUserDecrypt))
                {
                    WriteToRegistry(GetKey("User"), Cryptor.Encrypt(m_SQLUser, BaseSettings.CryptAlgorithmMode));
                    userChanged = true;
                ...
                if (sqlPasswordRegistry.Length > 0)
                {
                    ...
                    if (s.Length > 0)
                    {
                        sqlPasswordDecrypt = Cryptor.Decrypt(s, sqlUserDecrypt, BaseSettings.CryptAlgorithmMode);
                    }
                    //we must change password if user was changed
                    if (m_SQLPassword != null && (userChanged || !(m_SQLPassword.Equals(sqlPasswordDecrypt))))
                    {
                        WriteToRegistry(GetKey("Password"), Cryptor.Encrypt(m_SQLPassword, m_SQLUser, BaseSettings.CryptAlgorithmMode));
                        if (CredentialChange != null)
        ...
        public void SaveToFile(string configFile)
        {
            ...
            if(Utils.IsEmpty(m_SQLUser))
               m_Config.SetItem(GetKey("User"), "");
                ...
                m_Config.SetItem(GetKey("User"), Cryptor.Encrypt(m_SQLUser, BaseSettings.CryptAlgorithmMode));
            if (Utils.IsEmpty(m_SQLPassword))
                m_Config.SetItem(GetKey("Password"), "");
            else
                m_Config.SetItem(GetKey("Password"), Cryptor.Encrypt(m_SQLPassword, Utils.Str(m_SQLUser), BaseSettings.CryptAlgorithmMode));
            m_Config.Save();
