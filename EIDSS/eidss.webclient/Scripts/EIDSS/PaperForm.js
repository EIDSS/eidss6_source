var paperForm = {
    ...
    OnStartDateTimeChanged: function (e) {
        paperForm.OnDateTimeChanged(e, true, true);
    },
    OnEndDateTimeChanged: function (e) {
        paperForm.OnDateTimeChanged(e, false, true);
    },

    OnDateTimeChanged: function (e, isStartDateChanged, specialRules) {
        if (specialRules == null) specialRules = false;

        var sd = $('#StartDate').data('kendoDateTimePicker');
        var ed = $('#EndDate').data('kendoDateTimePicker');
        ...
        if (startDate == null) {
            if (isStartDateChanged == true) {
                if (oldStartDate == undefined)
                    oldStartDate = sd.element[0].defaultValue;
                if (oldStartDate != undefined) {
                    $('#StartDate').data('previous', oldStartDate);
                    sd.value(oldStartDate);
                }
                return;
            }
        ...
        if (endDate == null) {
            if (isStartDateChanged == false) {
                if (oldEndDate == undefined)
                    oldEndDate = ed.element[0].defaultValue;
                if (oldEndDate != undefined) {
                    $('#EndDate').data('previous', oldEndDate);
                    ed.value(oldEndDate);
                }
                return;
            }
