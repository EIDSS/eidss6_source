...
namespace eidss.webclient.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public FileResult AvianDiseaseOutbreaksForm()
        {
            ...
            {
                reportName = "Investigation Form For Avian Disease Outbreaks GG.pdf";
            }
            else if(EidssSiteContext.Instance.IsJordanMoACustomization)
            {
                reportName = "Investigation Form For Avian Disease Outbreaks JO.pdf";
            }
            else
            {
                reportName = "Investigation Form For Avian Disease Outbreaks.pdf";
            }
        ...
        public FileResult LivestockDiseaseOutbreaksForm()
        {
            ...
            if (EidssSiteContext.Instance.IsGeorgiaCustomization)
            {
                reportName = "Investigation Form For Livestock Disease Outbreaks GG.pdf";
            }
            else if (EidssSiteContext.Instance.IsJordanMoACustomization)
            {
                reportName = "Investigation Form For Livestock Disease Outbreaks JO.pdf";
            }
            else
            {
                reportName = "Investigation Form For Livestock Disease Outbreaks.pdf";
            }
        ...
        public ActionResult CovidReportUA()
        {
            ViewBag.GetReportActionName = "ShowCovidReportUA";
            ViewBag.Title = EidssMenu.Instance.GetString("CovidReportUA");
            var model = new UACov19ListModel()
            {
                CanWorkWithArchive = true,
                Language = ModelUserContext.CurrentLanguage
            };
            model.Address.IsSettlementVisible = true;

            return View("UA/CovidReportUA", model);
        }

        public ActionResult ShowCovidReportUA(UACov19ListModel model)
        {
            return PrepareReportResult(ProcessUACov19ListModel(model));
        }

        private static ReportResultParameters ProcessUACov19ListModel(UACov19ListModel model)
        {
            ReportResultParameters resultParameters;
            ...
            using (var wrapper = new ReportClientWrapper())
            {
                ...
                report = wrapper.Client.ExportUACovid(model);

                string fileName = "CovidReport" + '_' + model.GenerateName();
