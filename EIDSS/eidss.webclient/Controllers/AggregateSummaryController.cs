            ...
            return ObjectStorage.Using<IObject, List<long>>(aggrSum =>
                {
                    ...
                    if (aggrSum is HumanAggregateCaseSummary && (aggrSum as HumanAggregateCaseSummary).AggregateCaseListItems.Count > 0)
                        observations = (aggrSum as HumanAggregateCaseSummary).AggregateCaseListItems.Where(i => (!i.IsMarkedToDelete) && (i.GetValue(observationFieldName) != null)).Select(item => (long)item.GetValue(observationFieldName)).ToList();
                    if (aggrSum is VetAggregateCaseSummary && (aggrSum as VetAggregateCaseSummary).AggregateCaseListItems.Count > 0)
                        observations = (aggrSum as VetAggregateCaseSummary).AggregateCaseListItems.Where(i => (!i.IsMarkedToDelete) && (i.GetValue(observationFieldName) != null)).Select(item => (long)item.GetValue(observationFieldName)).ToList();
                    if (aggrSum is VetAggregateActionSummary && (aggrSum as VetAggregateActionSummary).AggregateCaseListItems.Count > 0)
                        observations = (aggrSum as VetAggregateActionSummary).AggregateCaseListItems.Where(i => (!i.IsMarkedToDelete) && (i.GetValue(observationFieldName) != null)).Select(item => (long)item.GetValue(observationFieldName)).ToList();
