...
namespace eidss.winclient.Administration
{
    public partial class PersonDetailPanel : BaseDetailPanel_Person
    {
        ...
        public PersonDetailPanel()
        {
            ...
            PersonLoginInfoPanel.ActionButtonStateChanged += LoginInfoGroupPanel_ActionButtonStateChanged;
        ...
        private void OrganizationChanged(object sender, EventArgs e)
        {
            ...
            if (person != null)
            {
                ...
                {
                    if ((send != null) && string.IsNullOrEmpty(send.Text))
                    {
                        cbOrganization.Properties.ForceInitialize();
        ...
        private void LoginInfoGroupPanel_ActionButtonStateChanged(object sender, ActionButtonEventArgs<IObject> e)
        {
            ...
            if (eidss.model.Core.EidssSiteContext.Instance.IsKazakhstanMoHCustomization
                && person.idfsOrganizationSite.HasValue
                && EidssUserContext.User.HasPermission(PermissionHelper.SelectPermission(EIDSSPermissionObject.User))
                && (person.LoginInfoList != null)
                && (person.LoginInfoList.Count(i => !i.IsMarkedToDelete) > 0)
               )
                LayoutCorrector.SetStyleController(this.txtTelephone, LayoutCorrector.MandatoryStyleController);
            else
                LayoutCorrector.SetStyleController(this.txtTelephone, LayoutCorrector.EditorStyleController);
        }
