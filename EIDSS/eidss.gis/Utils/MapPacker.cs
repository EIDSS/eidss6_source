    ...
    public static class MapPacker
    {
        ...
        public static void PostUnpackParseMapFile(string mapFilePath, string dataPath)
        {
            ...
            foreach (XmlNode fileNode in filenames)
            {
                ...
                if (Path.IsPathRooted(fileNode.InnerText))
                {
                    throw new ApplicationException(string.Format("Map File contains rooted path in one of the nodes: {0}", fileNode.InnerText));
                }
