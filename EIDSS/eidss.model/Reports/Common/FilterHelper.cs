...
namespace eidss.model.Reports.Common
{
    ...
    public class FilterHelper
    {
        ...
        public static long? GetDefaultSettlement(bool hasChe = false)
        {
            long? regionId;
            long? rayonId;
            long? settlementId;
            GetDefaultLocation(out regionId, out rayonId, out settlementId, hasChe);
            return settlementId;
        }
        ...
        public static void GetDefaultLocation(out long? regionId, out long? rayonId, out long? settlementId, bool hasChe = false)
        {
            regionId = null;
            rayonId = null;
            settlementId = null;
            ...
            if (locationTable.Rows.Count != 0)
            {
                ...
                if (!(dataRow["idfsSettlement"] is DBNull))
                {
                    settlementId = (long)dataRow["idfsSettlement"];
                }
            }
        }
        ...
        public static List<SelectListItemSurrogate> GetOutcomesList
            (string lang, int hacode, bool needEmptyItem)
        {
            var parameters = new Dictionary<string, object>
            {
                {"@LangID", lang},
                {"@type", (long) BaseReferenceType.rftOutcome},
                {"@HACode", hacode}
            };
            DataTable table = ExecSp("spReference_SelectLookup", parameters);
            List<SelectListItemSurrogate> result = table.Rows.Cast<DataRow>().Select(
                r => new SelectListItemSurrogate
                {
                    Text = r["name"].ToString(),
        ...
        public static List<SelectListItemSurrogate> GetExcelExportFormat()
        {
            var xlsxFormat = Enum.GetName(typeof(ReportExportType), ReportExportType.Xlsx);
            string[] formats = new string[1] { xlsxFormat };
            List<SelectListItemSurrogate> items = formats
                .Select(c => new SelectListItemSurrogate
                {
                    Text = c,
                    Value = c,
                    Selected = c == ReportExportType.Xlsx.ToString()
                })
                .ToList();
            return items;
        }
