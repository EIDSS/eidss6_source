...
namespace eidss.main.Login
{
    public partial class LoginForm : BvForm
    {
        ...
        {
            //Check for open Login form in case of redirection from failed EDS process
            var found = Application.OpenForms.OfType<LoginForm>().FirstOrDefault() as LoginForm;
            if (found != null)
            {
                BaseFormManager.Close(found, DialogResult.Cancel, FormClosingMode.NoSave, true);
            }
