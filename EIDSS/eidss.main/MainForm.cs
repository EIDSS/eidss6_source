...
namespace eidss.main
{
    public partial class MainForm : BvForm, IMainForm
    {
        ...
        internal bool m_triggerOnFormClosing = true;
        EidssSecurityManager manager;
        ...
        private bool RemoveParallelSessions(string userName, out bool hasParallelSessions)
        {
            hasParallelSessions = false;
            if (string.IsNullOrEmpty(userName))
                return false;

            var dc = ForcedDisconnectionClient.Instance;

            if (dc.HasParallelSessions(userName))
            {
                hasParallelSessions = true;
                var modal = new DisconnectParallelSessionsForm();
                modal.ShowDialog();
                var result = modal.DialogResult;
        ...
        private bool Login(bool needSave) 
        {
            if (!BaseFormManager.CloseAll(needSave))
                return true;
            if (m_ActivityMonitor != null)
            {
                m_ActivityMonitor.Enabled = false;
            }
            manager = new EidssSecurityManager();
            manager.LogOut();
            Visible = false;
            ...
            bool isok = false;
            Splash.HideSplash();
            var iMainForm = this as IMainForm;

            //TODO: check if needed to be moved or refactored
            if (EidssSiteContext.Instance.EnableDisconnection)
            {
                bool hasParallelSessions = false;
                bool doRemoveParallelSessions;
                doRemoveParallelSessions = RemoveParallelSessions(EidssUserContext.User.LoginName, out hasParallelSessions);

                if (hasParallelSessions)
                {
                    if (doRemoveParallelSessions)
                    {
                        //Parallel Sessions Tracking and Disconnection
                        var rc = ForcedDisconnectionClient.Instance;
                        rc.Disconnected += Call_LogOut;
                        rc.IsDisconnected();
                        rc.RemoveParallelSessions(EidssUserContext.User.LoginName);
                    }
                    else
                    {
                        iMainForm.LogOutRedirectToLogin(string.Empty, null, false);
                    }
                }
                else
                {
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (EidssSiteContext.Instance.EnableDisconnection)
                    ForcedDisconnectionClient.Instance.DisconnectMySession();
        ...
        */
        ...
        void IMainForm.LogOutRedirectToLogin(string errId, int? securityEv, bool showErrorForm)
        {
            var errorMessage = string.Empty;
            
            bool hasErrorMessage;
            hasErrorMessage = !string.IsNullOrEmpty(errId);
            
            if (hasErrorMessage)
            {
                errorMessage = EidssMessages.Get(errId);
                if (showErrorForm)
                    ErrorForm.ShowErrorDirect(errorMessage);
            }

            bool logSecurityEvent;
            logSecurityEvent = securityEv.HasValue && Enum.IsDefined(typeof(SecurityAuditEvent), securityEv.Value.ToString());
            if (logSecurityEvent)
            {
                SecurityAuditEvent securityEvItem = (SecurityAuditEvent) Enum.Parse(typeof(SecurityAuditEvent), securityEv.Value.ToString());
                EdsCommonHelper.ReflectResultToSecurityLog(securityEvItem, false, errorMessage, string.Empty);
            }
        ...
        private DisconnectedForm m_DisconnectedForm;

        private void Call_LogOut(object sender, EventArgs args)
        {
            //manager.LogOut();
            ForcedDisconnectionClient.Instance.Disconnected -= Call_LogOut;
            
            m_triggerOnFormClosing = false;
            m_DisconnectedForm = new DisconnectedForm(Localizer.SupportedLanguages[EidssUserContext.CurrentLanguage]);
            m_DisconnectedForm.ShowDialog();
            m_DisconnectedForm.Dispose();

            this.BeginInvoke((MethodInvoker)delegate() { this.Login(false); });
        }
