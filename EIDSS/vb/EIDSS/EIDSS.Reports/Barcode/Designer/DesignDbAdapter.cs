    ...
    public class DesignDbAdapter
    {
        ...
        public string GetBarcodeLayout()
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(ModelUserContext.Instance))
            {
                ...
                return string.IsNullOrEmpty(result)
                           ? string.Empty
                           : Cryptor.Decrypt(result, CryptorAlgorithm.Rijndael);
            }
        }
        ...
        public void SaveBarcodeLayout(string layout)
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(ModelUserContext.Instance))
            {
                ...
                command.Parameters.Add(new SqlParameter("@strBarcodeLayout", Cryptor.Encrypt(layout, CryptorAlgorithm.Rijndael)));
                command.ExecuteNonQuery();
            }
        }
