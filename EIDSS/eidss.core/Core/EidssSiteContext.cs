...
namespace eidss.model.Core
{
    ...
    public class EidssSiteContext
    {
        ...
        private bool? m_EnableDisconnection;
        public bool EnableDisconnection
        {
            get
            {
                if (!m_EnableDisconnection.HasValue)
                {
                    m_EnableDisconnection = false;
                    int intSetting = 0;
                    if (int.TryParse(GetGlobalSiteOption(GlobalSiteOption.ForcedDisconnectionEnable), out intSetting))
                    {
                        if (intSetting == 1)
                            m_EnableDisconnection = true;
                    }
                }
                return m_EnableDisconnection.Value;
            }
        }
        ...
        private int m_EdsAttemptCount = -1;
        public int EdsAttemptCount
        {
            get
            {
                if (m_EdsAttemptCount == -1)
                {
                    if (!int.TryParse(GetGlobalSiteOption(GlobalSiteOption.EdsAttemptCount), out m_EdsAttemptCount))
                        m_EdsAttemptCount = 3;
                }
                return m_EdsAttemptCount;
            }
        }

        private string m_EdsLinkForRecalledCheck = string.Empty;
        public string EdsLinkForRecalledCheck
        {
            get
            {
                if (string.IsNullOrEmpty(m_EdsLinkForRecalledCheck))
                {
                    m_EdsLinkForRecalledCheck = GetGlobalSiteOption(GlobalSiteOption.EdsLinkForRecalledCheck);
                    if (string.IsNullOrEmpty(m_EdsLinkForRecalledCheck))
                        m_EdsLinkForRecalledCheck = "http://ocsp.pki.gov.kz";
                }
                return m_EdsLinkForRecalledCheck;
            }
        }
