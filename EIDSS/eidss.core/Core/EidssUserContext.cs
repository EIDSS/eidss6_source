...
namespace eidss.model.Core
{
    ...
    public class EidssUserContext : ModelUserContext
    {
        ...
        public override int ParallelSessionsCount(string loginName, string clientID)
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(this))
            {
                int parallelSessionsCount = manager.SetSpCommand("dbo.spParallelSessionsCount",loginName, clientID).ExecuteScalar<int>();

                return parallelSessionsCount;
            }
        }

        public bool DeleteParallelSessions(string loginName, string clientId)
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(this))
            {
                int success = manager.SetSpCommand("dbo.spDeleteParallelSessions", loginName, clientId).ExecuteScalar<int>();
        ...
        public void DeleteLocalConnectionContext(string clientId)
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(this))
            {
                manager.SetSpCommand("dbo.spDeleteLocalConnectionContext", clientId).ExecuteScalar<int>();
            }
        }
        ...
        public bool IsDisconnected()
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(this))
            {
                int success = manager.SetSpCommand("dbo.spIsDisconnected", User.ID, EidssUserContext.ClientID).ExecuteScalar<int>();

                return success > 0;
            }
        }

        public bool IsDisconnected(object userId, string clientId)
        {
            if (userId == null)
                return false;
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(this))
            {
                int success = manager.SetSpCommand("dbo.spIsDisconnected", userId, clientId).ExecuteScalar<int>();
