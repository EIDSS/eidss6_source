...
namespace bv.winclient.BasePanel
{
    public class BaseFormManager
    {
        ...
        public static bool Close
            (IApplicationForm form, DialogResult result = DialogResult.Cancel, FormClosingMode closeMode = FormClosingMode.NoSave, bool doDisposeAnyway = false)
        {
            if (FormClosing)
            {
                return true;
            ...
            try
            {
                ...
                if (baseFormContainer != null)
                {
                    if (ModalFormCount > 0)
                    {
                        ...
                        if (doDisposeAnyway && (baseFormContainer != null) && (!baseFormContainer.IsDisposed))
                            baseFormContainer.Dispose();
                        disposed = true;
                    }
                ...
                if (!disposed)
                {
                    ...
                    {
                        var asyncResult = control.BeginInvoke((MethodInvoker)delegate() { control.Dispose(); });
        ...
        public static void LogOutRedirectToLogin(string errId, int? securityEv, bool showErrorForm = false)
        {
            if (MainForm == null)
                return;

            var iMainForm = MainForm as IMainForm;

            if (iMainForm == null)
                return;

            iMainForm.LogOutRedirectToLogin(errId, securityEv, showErrorForm);
        }
